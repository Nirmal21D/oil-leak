...
export default function CFARRadarPanel({
  ...
  const hasTargets = Boolean(darkVessels && darkVessels.length > 0);
  const target = hasTargets ? darkVessels[0] : null;
  ...
  return (
    <section id="cfar" className="bg-concrete-950 border-2 border-concrete-700 p-5 font-mono select-none text-concrete-100 space-y-4">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 border-b-2 border-concrete-700 pb-3">
        ...
        <span className={`stamp-tag font-bold ${
          hasTargets ? 'border-amber-500 text-amber-400' : 'border-concrete-700 text-concrete-400'
        }`}>
          {hasTargets ? 'SYNTHETIC DEMONSTRATION DATA // NOT OBSERVED REAL CONTACT' : 'CFAR RECON STANDBY'}
        </span>
      </div>

      {!hasTargets ? (
        <div className="bg-concrete-900 border border-concrete-700 p-6 text-center space-y-3">
          <span className="text-amber-500/80 font-bold text-xs uppercase tracking-widest block">
            [RADAR TELEMETRY STATUS]
          </span>
          <p className="text-xs text-concrete-300 font-bold">
            CFAR DATA UNAVAILABLE // NO UNMATCHED RADAR CONTACTS DETECTED IN SCENE
          </p>
          <p className="text-[11px] text-concrete-500 max-w-xl mx-auto leading-relaxed">
            In operational historical SAR mode, synthetic radar contacts are strictly suppressed to prevent non-physical suspect correlation. All attribution leads must originate from verified spatial AIS broadcasts or authentic CA-CFAR target clusters.
          </p>
        </div>
      ) : (
          ...
          <div className="md:col-span-2 bg-concrete-900 border border-concrete-700 p-4 space-y-3">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
              <div className="bg-concrete-950 p-3 border border-concrete-800">
                ...
                <span className="text-[9px] text-concrete-600 block mt-0.5">RCS: ~{target.rcs_db} dBsm</span>
              </div>
              ...
              <div className="bg-concrete-950 p-3 border border-concrete-800">
                ...
                <span className="text-[9px] text-concrete-600 block mt-0.5">ZERO PINGS SCANNED</span>
              </div>
              ...
              <div className="bg-concrete-950 p-3 border border-concrete-800">
                ...
                <span className="text-[9px] text-concrete-600 block mt-0.5">ESTIMATED INDEX</span>
              </div>
            </div>
          ...
          <div className="bg-concrete-900 border border-concrete-700 p-4 flex flex-col justify-between space-y-3">
            <div>
              ...
              <div className="space-y-2 text-xs text-concrete-400">
                ...
                <div>• Target ID: {target.dark_target_id}</div>
              </div>
            </div>
